...
public class BasketController {
    ...
    private Basket convertToBasketEntity(BasketResponse basketResponse) {
        ...
        basket.setItems(mapBasketItemResponsesToEntities(basketResponse.getItems(), basket));
        return basket;
    }

    private List<BasketItem> mapBasketItemResponsesToEntities(List<BasketItemResponse> itemResponses, Basket basket) {
        return itemResponses.stream()
                .map(itemResponse -> convertToBasketItemEntity(itemResponse, basket))
                .collect(Collectors.toList());
    }

    private BasketItem convertToBasketItemEntity(BasketItemResponse itemResponse, Basket basket) {
        ...
        basketItem.setBasket(basket); // Set the basket for the item
        return basketItem;
    }
}